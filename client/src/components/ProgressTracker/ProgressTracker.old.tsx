...
import { CheckCircle, Circle, Clock, Zap, Search, BarChart3, Table, Wifi, WifiOff } from 'lucide-react';
import { cn } from '@/lib/utils';
// WebSocket disabled due to persistent 1006 errors - using polling only
...
export default function ProgressTracker({ 
  ...
  // Fallback timer progression when WebSocket is not available
  useEffect(() => {
    // Only use fallback timers if WebSocket is not connected or real-time is disabled
    if (currentStep === 'discovery' && (!isConnected || !useRealTime)) {
      console.log('[ProgressTracker] Using fallback timers (WebSocket not available)');
      ...
      const timer1 = setTimeout(() => {
        ...
        setLocalProgress(50);
        setCurrentMessage('Researching products and gathering data...');
      }, 18000); // 18 seconds for discovery
      
      const timer2 = setTimeout(() => {
        setLocalStep('categorization');
        setLocalProgress(75);
        setCurrentMessage('Analyzing features with Kano methodology...');
      }, 35000); // 17 seconds for research
      
      const timer3 = setTimeout(() => {
        setLocalStep('table_creation');
        setLocalProgress(90);
        setCurrentMessage('Building your competitive analysis table...');
  ...
  }, [currentStep, isConnected, useRealTime]);
